...
class PolargraphVectorizer:
    ...
    def vectorize_image(
        self,
        image_data: bytes,
        settings: Optional[VectorizationSettings] = None,
        ...
        try:
            ...
            if output_dir and all_paths:
                ...
                # Generate unique SVG filename based on provided base name (if any)
    ...
    def create_svg_for_project(self, result: VectorizationResult, project_id: str, 
        ...
        try:
            ...
            name_root = (base_filename or "vectorized").strip() or "vectorized"
            svg_filename = f"{name_root}_{timestamp}.svg"
            result.source_image_name = result.source_image_name or base_filename
    ...
    def _generate_svg_content(self, result: VectorizationResult) -> str:
        ...
        settings_dict = asdict(result.settings_used)
        generated_ts = datetime.now().isoformat()
        source_name = result.source_image_name or "unknown"
        version = "1.0.0"

        svg_lines = [
            '<?xml version="1.0" encoding="UTF-8"?>',
            f'<!-- Generated by PolarVortex v{version} at {generated_ts} -->',
            f'<!-- Source image: {source_name} -->',
            f'<!-- Parameters: {json.dumps(settings_dict, separators=(",", ":"))} -->',
