...
def build_vpype_pipeline(svg_path: Path, paper: PaperSize, fit_mode: FitMode, output_path: Path) -> str:
    ...
    if fit_mode == "fit":
        place_cmd = f"scaleto {width}x{height} --fit --origin center"
    else:
        place_cmd = f"pagesize {width}x{height} --origin center translate {width/2} {height/2}"

    pipeline = (
        f'read "{svg_path}" | '
        f"{place_cmd} | "
        f'write gcode "{output_path}"'
    )
    return pipeline
